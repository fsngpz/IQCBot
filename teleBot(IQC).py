...
def start(update, context): #-> Function ini berguna untuk memulai bot dan mengecek apakah user diperbolehkan menggunakan bot ini atau tidak
    if update.effective_chat.username not in allowedUsernames:
        ...
        if not menu or menu[f"{update.effective_user.username}"] == '':
            ...
            buttons = [[KeyboardButton('Incoming Quality Control')]]
            ...
            context.bot.send_message(reply_markup=ReplyKeyboardMarkup(buttons), chat_id=update.effective_chat.id,
                                     parse_mode='Markdown',
                                     text=f"Hello {update.effective_user.first_name}, Welcome to *Incoming Quality "
                                          f"Control Menu System*!")
            update.message.reply_text(parse_mode='Markdown',
                                      text="To find the latest report of the Incoming, please send me Picture or "
                                           "File of the *GRN*")
    ...
    if update.effective_chat.username not in allowedUsernames:
        ...
        if pathPPT[f'{update.effective_user.username}'] != '' and 'Yes' == update.message.text:
            ...
            if fileName[f'{update.effective_user.username}'] == 'Collector':
                pathPPT[f'{update.effective_user.username}'] = r"\\Apckranefa01pv\apckr001\CKR\APCKRMFLPT001P\G_Drive\Groups\QA\SQT by QE\SQT PPT File\01. Barbie Collector.pptx"
            ...
            try:
                context.bot.send_document(chat_id=update.effective_chat.id, document=open(
                    rf"{pathPPT[f'{update.effective_user.username}']}",
                    "rb"), filename=fileName[f'{update.effective_user.username}'], reply_markup=ReplyKeyboardMarkup(buttons))
            except OSError:
                ...
                context.bot.send_message(parse_mode='Markdown',
                                         chat_id=update.effective_chat.id,
                                         text="`Please send me another Photo/LKP and try again`")
            except telegram.error.NetworkError:
                context.bot.send_message(parse_mode='Markdown',
                                         chat_id=update.effective_chat.id,
                                         text="`We are very sorry because we cannot getting the PPT File. Due too large size of the file :(`")
                context.bot.send_message(chat_id=update.effective_chat.id, parse_mode='Markdown',
                                 text="Key-In data for QC Molding System. Please select either the part *PASS* or *FAIL*!",
                                 reply_markup=ReplyKeyboardMarkup(buttons))
        ...
        if 'PASS' in update.message.text and Status[f"{update.effective_user.username}"] == 'PASS':
            ...
            buttons = [[InlineKeyboardButton("51-90", callback_data="51-90"), InlineKeyboardButton("91-150", callback_data="91-150")],
                       [InlineKeyboardButton("151-280", callback_data="151-280"), InlineKeyboardButton("280-500", callback_data="280-500")],
                       [InlineKeyboardButton("281-500", callback_data="281-500"),InlineKeyboardButton("501-1200", callback_data="501-1200")],
                       [InlineKeyboardButton("601-1200", callback_data="601-1200"),InlineKeyboardButton("1201-3200", callback_data="1201-3200")],]
            context.bot.send_message(chat_id=update.effective_chat.id, reply_markup=InlineKeyboardMarkup(buttons),
